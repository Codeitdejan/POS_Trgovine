...
namespace PCPOS.Report.Faktura_njem
{
    public partial class FakturaNjemackiReportForm : Form
    {
        public FakturaNjemackiReportForm()
        {
            InitializeComponent();
        }
        ...
        private void repFaktura_Load(object sender, EventArgs e)
        {
            ...
            string vpcplusporez = "RTL Price";
            try
            {
                if (ini.Read("VPCPLUSPOREZ", "vpcplusporez") == "1")
                {
                    vpcplusporez = "Whls.+VAT";
                }
            }
            catch
            {
            ...
            string valut = " Select fakture.id_valuta, valute.ime_valute AS valuta, fakture.id_fakturirati FROM fakture " +
                " LEFT JOIN valute ON valute.id_valuta=fakture.id_valuta WHERE fakture.broj_fakture='" + broj_dokumenta + "'" +
                " AND fakture.id_ducan='" + poslovnica + "' AND fakture.id_kasa='" + naplatni + "'";
            DTvalut = classSQL.select(valut, "valute").Tables[0];

            string drzava = classSQL.select("SELECT zemlja.zemlja FROM partners LEFT JOIN zemlja ON partners.id_zemlja = zemlja.id_zemlja WHERE id_partner = '" + DTvalut.Rows[0]["id_fakturirati"].ToString() + "'", "zemlja").Tables[0].Rows[0]["zemlja"].ToString();
            if (drzava == "")
            {
                drzava = "-";
            }
        ...
        private void FillRacun(string broj, string[] imeTablica)
        {
            ...
            string sql = "SELECT " +
                ...
                " '" + rabatSve + "' AS rabat," +
                //" " + imeTablice[0] + ".godina_fakture," +
                //" CAST (" + imeTablice[0] + ".broj_racuna AS nvarchar) +'/'+ CAST (" + year + " AS nvarchar) AS Naslov," +
        ...
        private void FillFaktura(string broj, string[] imeTablica)
        {
            ...
            double rabat;
            double mpcStavka;
            double mpcStavke_cisto;
            double rabatStavka = 0;
            double rabatStavka_cisto = 0;
            double pdvStavka = 0;
            double pdvStavka_cisto = 0;
            double osnovicaStavka = 0;
            double rabatSve = 0;
            double mpc = 0;
            double sveUkupno = 0;
            ...
            if (velep == "1")
            {
                for (int i = 0; i < DT.Rows.Count; i++)
                {
                    ...
                    vpc_cisto = Math.Round(Convert.ToDouble(DT.Rows[i]["vpc"].ToString()), 3);
                    kol = Math.Round(Convert.ToDouble(DT.Rows[i]["kolicina"].ToString()), 3);
                    porez = Math.Round(Convert.ToDouble(DT.Rows[i]["porez"].ToString()), 3);
                    rabat = Math.Round(Convert.ToDouble(DT.Rows[i]["rabat"].ToString()), 3);
                    ...
                    rabatStavka_cisto = Math.Round(vpc_cisto * kol * rabat / 100, 3);
                    mpcStavka = Math.Round(((vpc * kol - rabatStavka)), 3);
                    mpcStavke_cisto = Math.Round(((vpc_cisto * kol - rabatStavka_cisto)), 3);
                    pdvStavka = Math.Round(mpcStavka * (porez / 100), 3);
                    pdvStavka_cisto = Math.Round(mpcStavke_cisto * (porez / 100), 3);
                    ...
                    if (racunajTecaj)
                    {
                        StopePDVa(Math.Round((stopa * 100), 0), Math.Round(pdvStavka_cisto, 2), Math.Round(((vpc * kol) - rabatStavka), 2));
                    }
                    else
                    {
                        StopePDVa(Math.Round((stopa * 100), 0), Math.Round(pdvStavka_cisto, 2), Math.Round(((vpc_cisto * kol) - rabatStavka_cisto), 2));
                    }
                }
            }
            else
            ...
            string val = " Select fakture.id_valuta, valute.ime_valute AS valuta FROM fakture " +
             " LEFT JOIN valute ON valute.id_valuta=fakture.id_valuta WHERE fakture.broj_fakture='" + broj + "'";
            DataTable DTval = classSQL.select(val, "valute").Tables[0];

            string broj_slovima = "Slovima: " + broj_u_text_engl.PretvoriBrojUTekst(sveUkupno.ToString(), ',', "kn", "lp").ToString().ToLower();
            if (racunajTecaj) broj_slovima = "";
            if (racunajTecaj == true)
            {
                if (DTval.Rows[0]["valuta"].ToString() == "978 EUR")
                    broj_slovima = "Slovima: " + broj_u_text_engl.PretvoriBrojUTekst(sveUkupno.ToString(), ',', "€", "c").ToString().ToLower();
            ...
            string sql = "SELECT " +
                ...
                " partners.ime_tvrtke AS kupac_tvrtka," +
                " 'Delivery Date:' AS naziv_date1," +
                " 'Date of maturity:' AS naziv_date2," +
                " partners.adresa AS kupac_adresa," +
                " partners.id_grad AS id_kupac_grad," +
                " partners.id_partner AS sifra_kupac," +
            ...
            if (racunajTecaj)
            {
                ...
                u_kunama = u_kune_iz_valute.ToString("#0.00") + " kn";
            }
            else
            {
                tecaj = VratiTecaj(imeTablica[0], "broj_fakture", broj);
                ...
                u_kune_iz_valute = Math.Round(Convert.ToDecimal(cista_cijena_sveukupno / tecaj), 2);
                u_kunama = u_kune_iz_valute.ToString("#0.00") + " " + kn;
            }
...
imeTablica[0], broj, poslovnica, naplatni);
            ...
            string avio_registracija = null, avio_tip_zrakoplova = null, avio_maks_tezina_polijetanja = null;
            ...
            this.reportViewer1.LocalReport.EnableExternalImages = true;
            this.reportViewer1.LocalReport.SetParameters(new ReportParameter[] { p7, p18, p19, p20 });
